...
class LoginSystem {
    ...
    setupEventListeners() {
        ...
        setTimeout(() => {
            ...
            console.log('Password elements found:', !!usernameInput, !!passwordInput);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (usernameInput) {
        ...
        usernameInput.focus();
        console.log('Username input enabled');
    }
...
function showLoginNotification(message, type) {
    ...
    notification.style.cssText = 'position: fixed; top: 20px; right: 20px; padding: 15px 20px; border-radius: 8px; color: white; font-weight: 500; z-index: 10000; animation: slideInRight 0.3s ease; background: ' + (type === 'success' ? 'var(--accent-green)' : type === 'error' ? '#e74c3c' : '#3498db') + ';';
...
// Add CSS animations for login notifications
const loginAnimationStyles = document.createElement('style');
loginAnimationStyles.textContent = '@keyframes slideInRight { from { transform: translateX(100%); opacity: 0; } to { transform: translateX(0); opacity: 1; } } @keyframes slideOutRight { from { transform: translateX(0); opacity: 1; } to { transform: translateX(100%); opacity: 0; } }';
